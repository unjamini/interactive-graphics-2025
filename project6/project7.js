...
{
    const maxTimeStep = 0.01; // 100 times per sec for too big dt
    
    const numSteps = Math.ceil(dt / maxTimeStep);
    const subDt = dt / numSteps;
    
    for (let step = 0; step < numSteps; step++) {
        var forces = Array(positions.length);
        for (var i = 0; i < positions.length; i++) {
            forces[i] = gravity.copy();
        }
        ...
        for (var i = 0; i < springs.length; i++) {
            ...
            var forceMagnitude = 3 * stiffness * (Math.max(diff.len(), 0.001) - spring.rest);
            var relativeVelocity = v1.sub(v0);
            var dampingForce = damping * relativeVelocity.dot(diff.unit());
            
            var totalForce = forceMagnitude + dampingForce;
            
            var force = diff.unit().mul(totalForce);
            forces[spring.p0].inc(force);
            forces[spring.p1].dec(force);
        }

        // semi-implicit Euler
        for (var i = 0; i < positions.length; i++) {
            velocities[i].inc(forces[i].mul(subDt / particleMass));
            positions[i].inc(velocities[i].mul(subDt));
        }
        ...
        for (var i = 0; i < positions.length; i++) {
            ...
            const epsilon = 0.001; // push-out value

            for (const axis of ['x', 'y', 'z']) {
                if (p[axis] < -1) {
                    p[axis] = -1 + epsilon;
                    if (v[axis] < 0) v[axis] = -v[axis] * restitution;
                } else if (p[axis] > 1) {
                    p[axis] = 1 - epsilon; 
                    if (v[axis] > 0) v[axis] = -v[axis] * restitution;
                }
